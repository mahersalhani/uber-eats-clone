...
const OrdersScreen = () => {
  ...
  const fetchOrders = () => {
    DataStore.query(Order, (order) => order.status.eq("READY_FOR_PICKUP")).then(
      async (orders) => {
        // const restaurant = await orders[0].Restaurant;
        const ordersWithRestaurant = await Promise.all(
          orders.map(async (order) => {
            ...
            return { ...order, Restaurant: restaurant, User: user };
          })
        );
